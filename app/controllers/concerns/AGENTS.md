...
- resource URL은 index/create와 show/update/destroy를 제공하고, `enable_upsert`일 때만 `PUT /{resource_id}`를 등록한다. relationship linkage와 related-resource URL은 serializer에 선언된 관계에서 파생된다. `enable_writes = False`이면 create/update/upsert/destroy와 관계 mutation 라우트를 아예 등록하지 않고 읽기 라우트만 남긴다.
