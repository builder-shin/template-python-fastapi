...
- 이 디렉터리의 세 파일은 공통 `CrudActions`의 route 기반 FastAPI 회귀를 둔다. 내부 action을 직접 호출해 HTTP 계약을 대신하지 않는다.
- 공통 concern 회귀는 필요한 controller router와 exception handler를 등록한 최소 `FastAPI` 앱을 사용할 수 있다. 새 공개 자원의 factory·명시 route·OpenAPI 검증은 형제 `tests/test_<resource>_controller.py`에서 실제 `create_app()`과 `get_session` override로 수행한다.
