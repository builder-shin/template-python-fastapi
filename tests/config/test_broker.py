...
@pytest.mark.parametrize(
    ...
    [
        (None, "redis://localhost:6379/0"),
        ("redis://queue.example:6380/4", "redis://queue.example:6380/4"),
    ],
)
def test_configure_broker_sets_redis_broker(
    ...
    monkeypatch.setattr(broker_config.dramatiq, "set_broker", set_broker)

    configured = broker_config.configure_broker()

    redis_broker_factory.assert_called_once_with(url=expected_url)
